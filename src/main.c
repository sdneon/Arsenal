...
Window *my_window;
Layer *lay1Top, //red background
    *lay2Btm,   //red hexagons
    *lay3Title, //name of club
    *layCard,  //battery level indication: red: <= 10%, yellow <= 20%
    *layLogo,   //signature item in crest: artillery
    *layHourTens, *layHourOnes, //hour digits
    *layPm;     //pm indicator
static BitmapLayer
    *layBmHex,   //red hexagons for minutes
    *layBmTitle, //name of club
    *layBmLogo;  //signature item in crest: orange ball
...
static const GPathInfo CARD_PATH_INFO = {
  .num_points = 4,
  .points = (GPoint []) {{0, 0}, {11, 0}, {11, 15}, {0, 15}}
};
...
static void bt_handler(bool connected) {
    ...
    bitmap_layer_set_bitmap(layBmLogo,
        m_bBtConnected? m_spbmItem: m_spbmItemDim);
}
...
void update2Btm(Layer *layer, GContext *ctx) {
    ...
    layer_set_frame(layer, GRect(0, yTop, bounds.size.w, bounds.size.h));
}
...
void updateOnes(Layer *layer, GContext *ctx)
{
    ...
    // Stroke the path:
    graphics_context_set_stroke_width(ctx, THICKNESS);
    graphics_context_set_stroke_color(ctx, GColorOxfordBlue); //GColorDukeBlue); //GColorBlue);
...
void updateCard(Layer *layer, GContext *ctx)
{
    ...
    GRect r = GRect(halfWidth, 0, halfWidth, bounds.size.h);
    GColor mask = GColorFromRGBA(255, 255, 255, 100);
    //graphics_context_set_stroke_color(ctx, mask);
    graphics_context_set_fill_color(ctx, mask);
    graphics_fill_rect(ctx, r, 0, GCornerNone);
    */
...
#endif
    //APP_LOG(APP_LOG_LEVEL_DEBUG, "Time is %d:%d", h, m);
...
void handle_init(void)
{
    ...
    lay2Btm = layer_create(GRect(0, frame.size.h /2, frame.size.w, frame.size.h));
    layer_set_update_proc(lay2Btm, update2Btm);
    layer_add_child(root_layer, lay2Btm);
    layBmHex = bitmap_layer_create(layer_get_bounds(lay2Btm));
    m_spbmHex = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_HEX);
    bitmap_layer_set_bitmap(layBmHex, m_spbmHex);
    //bitmap_layer_set_background_color(layBmHex, GColorClear);
    bitmap_layer_set_compositing_mode(layBmHex, GCompOpSet);
    layer_add_child(lay2Btm, bitmap_layer_get_layer(layBmHex));

    lay3Title = layer_create(GRect(0, 10, frame.size.w, 31));
    layer_add_child(root_layer, lay3Title);
    layBmTitle = bitmap_layer_create(layer_get_bounds(lay3Title));
    m_spbmTitle = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_TITLE);
    bitmap_layer_set_bitmap(layBmTitle, m_spbmTitle);
    //bitmap_layer_set_background_color(layBmTitle, GColorClear);
    bitmap_layer_set_compositing_mode(layBmTitle, GCompOpSet);
    layer_add_child(lay3Title, bitmap_layer_get_layer(layBmTitle));

    //Create card
    m_spathCard = gpath_create(&CARD_PATH_INFO);
    // Rotate 30 degrees clockwise:
    gpath_rotate_to(m_spathCard, TRIG_MAX_ANGLE / 360 * 45);
    // Translate to top right quadrant of ball:
    gpath_move_to(m_spathCard, GPoint(130, 95));
    ...
    layLogo = layer_create(GRect(79, 102, 63, 63));
    ...
    m_slaytxtPm = text_layer_create(
        GRect(frame.size.w * 4 / 5 + 6, 41, 20, 20));
...
void handle_deinit(void)
{
    ...
    gbitmap_destroy(m_spbmTitle);
    gbitmap_destroy(m_spbmHex);
    gbitmap_destroy(m_spbmItemDim);
    gbitmap_destroy(m_spbmItem);
    bitmap_layer_destroy(layBmTitle);
    bitmap_layer_destroy(layBmHex);
